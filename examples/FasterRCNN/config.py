...
_C.BACKBONE.RESNET_NUM_BLOCKS = [3, 4, 6, 3]   # for resnet50, number of each type of block to stack
...
# CONF: Cascade R-CNN, only available in FPN mode
...
_C.freeze(frozen=True)  # avoid typo / wrong config keys
...
def to_hashable(d):
    for x in d:
        if isinstance(d[x], list):
            d[x] = tuple(d[x])
            print(x, '  >>>  convert to hashable')
        elif isinstance(d[x], AttrDict):
            to_hashable(d[x].__dict__)
...
def finalize_configs(is_training):
    ...
    _C.freeze(frozen=True)
    # logger.info("Config: ------------------------------------------\n" + str(_C.to_yaml(output_path=None)))
    logger.info("Config: ------------------------------------------\n" + str(_C))
